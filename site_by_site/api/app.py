...
def vendors():
    ...
    out.sort(key=lambda r: r["n"], reverse=True)  # sort in Python to avoid dialect quirks
    return jsonify(out)
...
def get_job(vendor, key):
    with engine.begin() as conn:
        row = conn.execute(
            sa.text(f'SELECT * FROM {TABLE_NAME} WHERE "Vendor"=:v AND "Dedupe Key"=:k'),
            {"v": vendor, "k": key},
        ).mappings().first()
    if not row:
...
def start_run():
    ...
    RUNS[rid] = {"status": "queued", "created_at": datetime.datetime.utcnow().isoformat(),
                 "args": payload, "stdout": ""}
...
@app.get("/runs")
def list_runs():
    items = [{"id": k, **v, "stdout": None} for k, v in sorted(RUNS.items(), key=lambda x: x[0], reverse=True)]
